...
<html>
  			 ...
  			 function showCoords(obj){ 
  			 	...
  			 	if(parseInt(obj.w) > 0){
  			 		...
  			 		*/
  			 		...
  			 		$("#hidden_x1").val(Math.round(rx * obj.y));
  			 		$("#hidden_y1").val(Math.round(rx * obj.x));
  			 		$("#hidden_width").val(Math.round(rx * $("#targetImg").width()));
  			 		$("#hidden_height").val(Math.round(rx * $("#targetImg").height()));
  			 	}
  			 }
  		});
  	</script>
  ...
  <body>
  	<div style="float:left;">
  		<form action="servlet/CropServlet" method="post">
		  	<img src="${requestScope.uploadSaveUrl }" id="targetImg"/>
		  	<input type="hidden" name="fileSaveUrl" value="${requestScope.uploadSaveUrl }">
		  	<br>
		  	<span>
		  		...
		  		裁切宽高度：<input type="text" name="width" id="w" style="width: 50px;"><input type="text" name="height" id="h" style="width: 50px;"><br>
		  		<input type="hidden" name="x1" id="hidden_x1" >
		  		<input type="hidden" name="y1" id="hidden_y1" >
		  		<input type="hidden" name="width" id="hidden_width" >
		  		<input type="hidden" name="height" id="hidden_height" >
		  	</span>
		  	<br>
		  	<input type="submit" value="裁切">
  		</form>
  	</div>
  	<!-- 
			...
			<tr>
				...
				<td>
					<div id="cropSpan" style="overflow:hidden;width:283px;height:340px;">
				  		<img id="cropImg" src="${requestScope.uploadSaveUrl }" />
				  	</div>
				</td>
				<td rowspan="2">
